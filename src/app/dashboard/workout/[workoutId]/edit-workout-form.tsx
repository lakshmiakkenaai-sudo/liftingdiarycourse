...
type WorkoutData = {
  ...
  notes: string | null
  startedAt: Date
  exercises: { id: number; exerciseId: number; name: string; sets: { id: number; setNumber: number; reps: number | null; weight: string | null }[] }[]
}
  ...
  const [exerciseEntries, setExerciseEntries] = useState<ExerciseEntry[]>(() =>
    workout.exercises.map((e) => ({
      ...
      sets: e.sets.length > 0
        ? e.sets.map((s) => ({ reps: s.reps != null ? String(s.reps) : "", weight: s.weight ?? "" }))
        : [emptySet()],
    })),
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <Input id="title" name="title" placeholder="e.g. Push Day" defaultValue={workout.title ?? ""} />
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <Input
          ...
          name="startedAt"
          type="date"
          defaultValue={format(workout.startedAt, "yyyy-MM-dd")}
          required
        />
      </div>
